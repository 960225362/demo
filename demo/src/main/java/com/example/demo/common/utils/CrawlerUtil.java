...
/**
 * @author huyue01@sinovatech.com 2019/8/25 11:08
 */
@Slf4j
public class CrawlerUtil {
    public static JSONObject getHttpJson(String url, int comeType) {
        InputStream is = null;
        ...
        try {
            ...
            if (connection.getResponseCode() == 200) {
                ...
                return getJsonString(bos.toString(), comeType);
            }
        } catch (IOException e) {
            log.info("获取getHttpJson数据异常");
        } finally {
            if (null != is) {
                try {
                    is.close();
                } catch (IOException e) {
                    log.info("关闭流【InputStream】异常");
                }
            }
            if (null != bos) {
                try {
                    ...
                    log.info("关闭流【ByteArrayOutputStream】异常");
                }
            }
        }
        return null;
    ...
    private static JSONObject getJsonString(String str, int comeType) {
        if (comeType == 1) {
            ...
            return new JSONObject(Boolean.parseBoolean(strNew.toString()));
        }
        return null;
    }
    ...
    public static List<Jobs> getData(String entity) {
        ...
        if (!StringUtils.isEmpty(complany)) {
...
//        JSONObject object = getHttpJson("https://movie.douban.com/j/new_search_subjects?sort=U&range=0,10&tags=&start=0", 1);
